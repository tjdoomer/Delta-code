...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile('.delta', 'config.yaml');
      await createTestFile('.delta', 'logs.json');
  ...
  describe('with deltaignore', () => {
    it('should ignore deltaignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.deltaignore'),
        'ignored.txt\nnode_modules/\n.delta/\n!/.delta/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectDeltaIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.deltaignore'),
        'ignored.txt\nnode_modules/\n.delta/\n!/.delta/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectDeltaIgnore: false,
          respectGitIgnore: true, // Explicitly disable delta ignore only
        },
      });
      expect(structure).toContain('ignored.txt');
