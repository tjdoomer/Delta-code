...
describe('summarizers', () => {
  ...
  beforeEach(() => {
    ...
    (mockDeltaClient.generateContent as Mock) = vi.fn();
  ...
  describe('summarizeToolOutput', () => {
    it('should return original text if it is shorter than maxLength', async () => {
      ...
      expect(mockDeltaClient.generateContent).not.toHaveBeenCalled();
    });
    ...
    it('should call generateContent if text is longer than maxLength', async () => {
      ...
      (mockDeltaClient.generateContent as Mock).mockResolvedValue({
      ...
      expect(mockDeltaClient.generateContent).toHaveBeenCalledTimes(1);
    ...
    it('should return original text if generateContent throws an error', async () => {
      ...
      (mockDeltaClient.generateContent as Mock).mockRejectedValue(error);
    ...
    it('should construct the correct prompt for summarization', async () => {
      ...
      await summarizeToolOutput(longText, mockDeltaClient, abortSignal, 1000);
...
`;
      const calledWith = (mockDeltaClient.generateContent as Mock).mock
        .calls[0];
