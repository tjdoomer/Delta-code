...
export function createShowMemoryAction(
  ...
  return async () => {
    ...
    const fileCount = config.getDeltaMdFileCount();
    ...
    if (currentMemory && currentMemory.trim().length > 0) {
      ...
      addMessage({
        ...
        content:
          fileCount > 0
            ? 'Hierarchical memory (DELTA.md or other context files) is loaded but content is empty.'
            : 'No hierarchical memory (DELTA.md or other context files) is currently loaded.',
        timestamp: new Date(),
      });
    }
  };
