...
The tool appends the provided `fact` to your context file in the user's home directory (`~/.delta/DELTA.md` by default). This filename can be configured via `contextFileName`.
...
```
save_memory(fact="The project I'm currently working on is called 'delta-code'.")
```
