...
exporters:
  googlecloud:
    project: "${projectId}"
    metric:
      prefix: "custom.googleapis.com/delta_cli"
    log:
      default_log_name: "delta_cli"
  debug:
    verbosity: detailed
service:
  telemetry:
...
async function main() {
  ...
  console.log(
    '\n🚀 To send telemetry, run the Delta Code in a separate terminal window.',
  );
  ...
  console.log(
    `   - Logs: https://console.cloud.google.com/logs/query;query=logName%3D%22projects%2F${projectId}%2Flogs%2Fdelta_cli%22?project=${projectId}`,
  );
  console.log(
